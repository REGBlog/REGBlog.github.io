...
function newWord() {
  // Clear existing letters and reset game state
  ...
  // Use a hash function to generate an index from the date string
  const dailyIndex = hashString(dateString) % words.length;

  // Select the word for the day
  word = words[dailyIndex].split("");

  // Uncomment below to see the word in the console (for debugging purposes)
  console.log("Today's word:", word.join(""));
}

// Simple hash function to get an integer from a string
...
function shareResults() {
  ...
  // Loop through each entry and generate the game state
  entries.forEach((entry) => {
    Array.from(entry.children).forEach((letter) => {
      // Check the class of each letter and append the corresponding emoji
      if (letter.classList.contains('correct')) {
        gameState += '🟩';
      } else if (letter.classList.contains('wrong-position')) {
        gameState += '🟨';
      } else if (letter.classList.contains('absent')) {
        gameState += '⬜';
      } else {
        gameState += '⬜'; // Default for letters not yet guessed or empty
      }
    });
